...
<!-- version: 0.3.0 | outputs: memory/market_insights/<niche>-<platform>-<YYYY>-<MM>.md | prompts: skills/trend-analysis/prompts/ -->
...
- Topic or category to analyze (e.g., "nutrition apps", "personal finance", "AI automation agencies")
- The likely `business_model` (`b2c | prosumer | b2b-smb | b2b2c`) from `idea.md`, the user profile, or the user; it decides which platform set to recommend
- Platform prompt templates from `skills/trend-analysis/prompts/` (see Prompts section below)

## User Interaction

Before executing, ask the user which platforms to include in the analysis. Recommend a default set by business model and wait for their answer before proceeding:

| Business model | Recommended platforms | Why |
|---|---|---|
| `b2c` | TikTok, Reddit, App Store, Web Search | Consumer demand shows up as content, community complaints, and app rankings |
| `prosumer` | Reddit, Web Search, B2B communities, X/Twitter | Individual professionals talk in operator communities and search with intent |
| `b2b-smb`, `b2b2c` | Web Search, B2B communities, X/Twitter | Business buyers reveal demand in reviews (G2, Capterra), operator forums (HN, Indie Hackers, r/msp), LinkedIn, and pricing pages; TikTok and App Store carry little signal |
...
> 6. **B2B communities** — G2/Capterra reviews, Hacker News, Indie Hackers, operator subreddits, LinkedIn, vendor pricing pages (recommended for business buyers)
> 7. **All of the above** — full multi-platform analysis (recommended for a new niche)
...
| `prompts/b2b-communities.md` | Business-buyer review platforms, operator communities, LinkedIn, pricing pages | `[NICHE]` |
...
| B2B communities | Buyer complaints in reviews, operator workarounds, revenue posts, ICP counts, price anchors | G2, Capterra, Hacker News, Indie Hackers, operator subreddits, LinkedIn, vendor pricing pages, partner directories |
...
5. Write findings to a new file: `memory/market_insights/<niche>-<platform>-<YYYY>-<MM>.md`. The narrative must end with a `## Sources` section listing every URL consulted as a markdown link; downstream skills copy these URLs into their own `sources` arrays, and a file with no URLs fails `python tests/validate_memory.py`.
6. Append a row for the new file to the index table in `memory/market_insights/README.md`.

One file per platform run. To cover all platforms, run this skill once per platform and produce one file each.

### When a platform returns nothing usable

Some platforms have no signal for some niches. A search of climbing communities for training discussion returns peer-reviewed hangboard studies; a search of TikTok for compliance tooling returns nothing at all. When that happens:

1. **Do not write a file.** An insight file with no community evidence in it is worse than no file, because downstream skills treat it as researched ground.
2. **Do not substitute a different platform's data** under the empty platform's name.
3. Record the attempted queries and the outcome in the `inputs_missing` field of whichever skill consumes the analysis, and in `scores.json.missing_inputs`.
4. Lower `score_confidence` in `idea-scoring` by one level if the missing platform was one of the two primary sources for the business model.
5. Say so plainly when presenting: which platform was attempted, what came back, and what was used instead.

Two platforms attempted and one usable is a normal outcome, not a failure. Fabricating the second is the failure.
